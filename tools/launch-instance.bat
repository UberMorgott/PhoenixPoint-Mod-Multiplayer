...
REM  Self-contained. Master lives at Multiplayer2\tools\launch-instance.bat;
...
REM    4. SYNCS Mods\ from the main install + workshop by running sync-mods.ps1
REM       for THIS instance, so a copy can never start on stale mods. Real file
REM       COPIES, never junctions: three instances sharing one physical TFTV.log
REM       made TFTV abort its init on whichever lost the race (RCA 2026-07-15).
REM       Sync failure = LOUD ABORT, never a silent stale launch.
...
REM         "sync" = do steps 1-5 only (dll + mods + appid + id), do NOT launch.
...
set "SYNC_PS1=E:\DEV\PhoenixPoint\Multiplayer2\tools\sync-mods.ps1"
...
REM     rmdir WITHOUT /s removes only the link, never the target's contents.
dir /AL /B . 2>nul | findstr /I /X "Mods" >nul && rmdir "Mods"
if not exist "Mods" mkdir "Mods"

REM --- drop empty real dirs and dead junctions (rmdir handles both) ------------
REM     Leftovers from folder-copying an instance or from an uninstalled mod.
REM     A dir that is still a source mod gets refilled by the sync right after.
...
REM --- sync THIS instance's mods from the main install + workshop --------------
REM     ABORT (not prompt) on failure: an unattended stale launch surfaces later
REM     as a phantom desync bug that costs far more than re-running this script.
if not exist "%SYNC_PS1%" (
    echo.
    echo ############## MOD SYNC UNAVAILABLE - NOT LAUNCHING ##############
    echo  Script not found: "%SYNC_PS1%"
    echo ##################################################################
    pause
    exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%SYNC_PS1%" -Instance "%CD%"
if errorlevel 1 (
    echo.
    echo ############## MOD SYNC FAILED - NOT LAUNCHING ###################
    echo  This instance would run STALE mods. Fix the errors above, re-run.
    echo ##################################################################
    pause
    exit /b 1
)
