...
- **HEAD = WA-2 GeoSiteChannel optional tails (2026-07-05; haven `c04382f` + alien-base/excavation/mission-drift `b8734c5`), NOT pushed.** 1422/1422 tests (1415 unit + 7 bridge). DLL SHA256 `CAE45072653AD1A9501E49ABA28E04A0C029B0BCDF49C0E1BE87B9F57A4A41FF` deployed (junction covers both instances). Ch#5 wire gains the versioned extras block (haven/alien-base/excavation tails; no-tail payload byte-identical to pre-WA-2); GeoMap subs 10→15 events + 2 GeoPhoenixFaction excavation events; mission-drift throttled dirty via `GeoUpdateableMission.Update` postfix. In-game soak pending (WA-2 checklist in the row above — plus WA-1/Batch-1/Batch-2 checklists still open).
- Prior HEAD = WA-1 world-activity MIST + BEHEMOTH (2026-07-05; mist ch#8 `bcf75f0` + behemoth `eef1c54`). 1391/1391 tests (1384 unit + 7 bridge). DLL SHA256 `57B5D753409AED9287D8CA583B92C1FA334E4049C64B9701F46943256165AD2C` deployed (junction covers both instances). State channels now #1-6 + **#8 mist** (#7 RESERVED for P7 objectives); behemoth rides 0xA5 + ch#6 under reserved key `FNV("__behemoth")`. In-game soak pending (WA-1: mist edge advances on client; behemoth visible/moves/hides-on-submerge/despawns-on-kill; vehicle mirror no-regress — plus Batch-1/Batch-2 checklists still open).
- Prior HEAD = popup-mirror Batch-2 P3+P6 (2026-07-05; P3 `00a4b2d` + P6 `a8ba2f5`). 1367/1367 tests (1360 unit + 7 bridge). DLL SHA256 `792F715F2010B7446CF071D58021998A4DE724E0FC53406BA6AAAB4FFFC30D5D` deployed (junction covers both instances). 0x69 whitelist = {0,2,4,6,11,14,15,20,25,26,28,34,36,38} ∪ outcomes {1,3,5,12,16,21,27,29,35,37}; blocking unchanged = {0,2,4,11,15,20,26,28,34,36} (outcomes NON-blocking); mandatory client-lock = {15,11,28}. Envelope adds `0xA8 GeoHarvestFloat`. In-game soak pending (spec Batch-2 checklist; Batch-1 checklist still open).
- Prior HEAD `98e9f3f` (2026-07-05, popup-mirror Batch-1 P1+P2): 1295/1295 tests (1288 unit + 7 bridge). DLL SHA256 `4AB26D8EEA9DED6791ED8D8AF6BAC4EA36357E60A89521AC9BBB3066FEC4C03D` deployed (junction covers both instances). 0x69 whitelist = {0,2,4,6,11,14,15,20,25,26,28,34,36,38}; blocking = {0,2,4,11,15,20,26,28,34,36}. In-game soak pending (spec Batch-1 checklist).
