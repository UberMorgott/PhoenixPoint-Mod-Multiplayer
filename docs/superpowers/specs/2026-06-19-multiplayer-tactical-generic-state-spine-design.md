# Multiplayer — Tactical Generic State-Spine: Design Spec (host↔client full-state replication)
...
> Mod tactical sync: `Multiplayer\src\Sync\Tactical\`. Existing per-action surfaces 0x80–0x8D (`TacticalSyncSurfaces.cs`).
> Consistency reference (geoscape side): `docs\superpowers\specs\2026-06-17-multiplayer-full-geoscape-replication-feasibility.md` §5 "RETIRE per-domain channels surface-by-surface".
...
All under `Multiplayer\src\Sync\Tactical\`. The spine calls these EXISTING helpers; it does not re-derive engine APIs.
...
## Cross-refs
- Existing tactical surfaces + rail: `Multiplayer\src\Sync\Tactical\TacticalSyncSurfaces.cs`, `TacticalDeploySync.cs` (`HandleTacticalEnvelope`).
...
- Geoscape consistency (retire surface-by-surface): `docs\superpowers\specs\2026-06-17-multiplayer-full-geoscape-replication-feasibility.md` §5/§7.
- Roadmap tracker: `Multiplayer\docs\COOP-SYNC-ROADMAP.md`.
- Prior tactical replication design (per-action, being pivoted): `docs\superpowers\specs\2026-06-17-multiplayer-tactical-replication-design.md`.
- MEMORY rules in force: `multiplayer-full-state-replication`, `dont-replace-working-architecture` (minimal targeted change, retire one at a time), `pp-serializer-context-and-pump` (status-restore path uses the configured Serializer if going via InstanceData).
