...
- **DeploymentPreparing** — shared mission-preparation occurrence created by Start and inserted at the head of every durable session player's inbox.
...
| Urgent ambush/deployment interrupt | full occurrence tuple and registered native state/modal/raiser family | host / priority interrupt | creation-set entitlement; Geoscape gate only; may preempt only an ordinary window already open there | successful native content/result presentation marks `Read`; the registered family's native acknowledge/Cancel then marks `Dismissed`; a supported Start follows the exceptional-offer transition below | authoritative mission expiry/destruction or loss of its last valid source | queued/read restore; open becomes suspended before preemption/level teardown and restores its checkpoint; invalidation removes queued/open/suspended/read/tactical-held carriers and closes native UI without Cancel callback |
...
| DeploymentPreparing | successor occurrence identity plus mission and current live-source set | host / highest deployment interrupt | new entitlement for every durable session player at Start, including players who dismissed the predecessor offer; Geoscape gate only | edits and first valid launch are host-serialized; Back enters `Deferred` and is nonterminal; successful launch or global invalidation marks `Removed` and releases every entitlement | mission expiry/destruction/completion, successful launch, or revalidation proving no valid source remains | queued/open/suspended/deferred/tactical-held state restores from current campaign facts; source loss prunes/repaints; terminal teardown closes every carrier without Cancel/back callbacks |
...
- The approved priority families are registered urgent ambush/deployment interrupts, exceptional mission offers, and `DeploymentPreparing`; every other classified mirrored family is ordinary. A missing registration is a loud DWI-18 failure, never an inferred priority or ephemeral fallback.
...
- Start atomically creates one new shared `DeploymentPreparing` occurrence at the head of every durable session player's inbox before consuming any offer carrier.
- The successor grants a fresh entitlement to every durable session player, including a player whose predecessor offer is already locally terminal; no offer lifecycle state is inherited by the new occurrence.
...
- Registered urgent ambush/deployment interrupts and exceptional mission offers are priority interrupts; `DeploymentPreparing` is the highest deployment interrupt. All sit ahead of ordinary windows in every inbox.
...
- At occurrence creation, the entitlement set is the durable player identities and membership epochs currently belonging to the campaign session, including disconnected/loading/tactical peers. A later connection id or roster slot never creates a new identity.
- A late join receives active occurrences only when the authoritative campaign join transaction explicitly enrolls its new membership epoch and grants those entitlements; otherwise creation-set membership is preserved. Rejoining the same durable identity resumes its existing epoch/lifecycle rather than duplicating backlog.
...
|---|---|---|
| DWI-01 | One host raise grants one stable occurrence entitlement to every session player, including tactical/disconnected/non-Geoscape peers. | retransmit dedupes; a new trigger for same event+subject remains distinct |
...
| DWI-10 | Any peer Start atomically creates one new `DeploymentPreparing` and fresh entitlement at every current durable session player's inbox head, including players whose offer is terminal. | concurrent Starts dedupe; failed creation leaves offer intact; predecessor Cancel state is not inherited |
...
| DWI-18 | Every mirrored campaign-window family is classified by registered native state/modal/raiser; local-answer windows are explicitly local-only. | unknown family fails coverage loudly rather than inferring priority, dropping, or displaying ephemerally |
...
| DWI-22 | Membership creation, late enrollment, reconnect, and authoritative removal use durable identity plus membership epoch without human ACK. | disconnect/AFK never ends membership; removed epoch cannot block compaction |
