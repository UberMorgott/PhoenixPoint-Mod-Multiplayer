# Multipleer Increment 1 — Rail Unification (Slice 1) Implementation Plan
...
types), existing `NetworkMessage` + `SyncProtocol` (BinaryWriter/Reader) wire layer, xUnit (`Multipleer.Tests`,
...
- Existing regression tests: `TacticalLiveSeqTests` + `TacticalIntentDedupTests` in
  `Multipleer.Tests/TacticalLiveCodecTests.cs:141-237` — **these are the green gate for the reparent (Task 3)**.
...
- `TacticalDeploySync.ArmInboundHook()` (`src/Sync/Tactical/TacticalDeploySync.cs:660-661`) =
  `SurfaceRouter.TacticalInbound = HandleTacticalEnvelope`; called once in `MultipleerMain.OnModEnabled`
  (`src/MultipleerMain.cs:47`). `HandleTacticalEnvelope(byte surfaceId, byte[] payload)` (`:673-804`) is a
...
**Build / test**
- Mod assembly auto-globs `src/**/*.cs` (only `Multipleer.Tests/**` is `<Compile Remove>`d) → new `src` files are
  auto-included (`Multipleer/Multipleer.csproj:17-22`). Build: `dotnet build Multipleer\Multipleer.csproj`.
- The test project does **NOT** auto-glob src (`EnableDefaultCompileItems=false`); each pure file is an explicit
  `<Compile Include="..\src\...">` link (`Multipleer/Multipleer.Tests/Multipleer.Tests.csproj`). Already linked:
...
- Test run: `dotnet test Multipleer\Multipleer.Tests\Multipleer.Tests.csproj` (single class:
  `... --filter FullyQualifiedName~SurfaceSeqTests`).
- Commit (inner repo, `main`, local only — `multipleer-commit-workflow`):
  `git -C E:/DEV/PhoenixPoint/Multipleer add -A && git -C E:/DEV/PhoenixPoint/Multipleer commit -m "<msg>"`.
...
| `Multipleer.Tests/Multipleer.Tests.csproj` | **Modify** | Link the three new pure files. |
| `Multipleer.Tests/SurfaceSeqTests.cs` | **Create** | Unit tests for `SurfaceSeq`. |
| `Multipleer.Tests/IntentDedupTests.cs` | **Create** | Unit tests for `IntentDedup`. |
| `Multipleer.Tests/GeoRailGateTests.cs` | **Create** | Pins the shipped default OFF. |
| `Multipleer.Tests/SurfaceRouterGeoscapeTests.cs` | **Create** | Router geoscape-hook dispatch + tactical precedence + drop. |

---

## Task 1 — Shared `SurfaceSeq` (UNIT-TESTED)

**Files:** Create `src/Network/Sync/SurfaceSeq.cs`; Create `Multipleer.Tests/SurfaceSeqTests.cs`; Modify
`Multipleer.Tests/Multipleer.Tests.csproj`.

- [ ] **Red — write the failing test.** Create `Multipleer.Tests/SurfaceSeqTests.cs`:
...
- [ ] **Add the link** in `Multipleer.Tests/Multipleer.Tests.csproj` immediately after the `RequestDedup.cs`
...
- [ ] **Run (red):** `dotnet test Multipleer\Multipleer.Tests\Multipleer.Tests.csproj --filter FullyQualifiedName~SurfaceSeqTests`
...
**Files:** Create `src/Network/Sync/IntentDedup.cs`; Create `Multipleer.Tests/IntentDedupTests.cs`; Modify
the test csproj.

- [ ] **Red — write the failing test.** Create `Multipleer.Tests/IntentDedupTests.cs`:
...
`TacticalLiveSeqTests` + `TacticalIntentDedupTests` (`Multipleer.Tests/TacticalLiveCodecTests.cs:141-237`) are
...
```
  ...
  resolve from `Multipleer.Sync.Tactical` exactly like the file's existing `Network.Sync.SurfaceRouter` references —
  no new `using` needed.)

- [ ] **Run (full suite, the regression gate):** `dotnet test Multipleer\Multipleer.Tests\Multipleer.Tests.csproj`
...
- [ ] **Build the mod** to confirm the live consumers still compile (they reference `TacticalDeploySync.LiveSeq`/
  `.IntentDedup` which keep their subclass types): `dotnet build Multipleer\Multipleer.csproj` → **`Build succeeded`**.
...
**Files:** Create `src/Network/Sync/GeoRailGate.cs`; Create `Multipleer.Tests/GeoRailGateTests.cs`; Modify the test csproj.

- [ ] **Red — write the failing test.** Create `Multipleer.Tests/GeoRailGateTests.cs`:
...
`Multipleer.Tests/SurfaceRouterGeoscapeTests.cs`. (`SurfaceIds.cs` + `SurfaceRouter.cs` are already linked in the test csproj.)

- [ ] **Red — write the failing test.** Create `Multipleer.Tests/SurfaceRouterGeoscapeTests.cs`. (All router tests
...
- [ ] **Build the mod:** `dotnet build Multipleer\Multipleer.csproj` → **`Build succeeded`**.
...
(`multipleer-second-instance-setup`).
...
```csharp
        ...
        private bool HandleGeoscapeEnvelope(byte surfaceId, byte[] payload)
        {
            if (surfaceId == SurfaceIds.GeoWallet)
            {
                ...
                catch (Exception ex) { Debug.LogError("[Multipleer] geo wallet envelope failed: " + ex.Message); }
                return true;
            }
            return false;
        }
...
- [ ] **Build + full test suite (no regression):** `dotnet build Multipleer\Multipleer.csproj` → `Build succeeded`;
  `dotnet test Multipleer\Multipleer.Tests\Multipleer.Tests.csproj` → **`Passed!  - Failed: 0`** (809 total).
...
- [ ] **In-game verification B (gate ON = new rail delivers).** Set `GeoRailGate.Enabled = true`, recompile + deploy,
  ...
  `Debug.Log("[Multipleer] geo wallet via envelope")` in `HandleGeoscapeEnvelope` to confirm the envelope path is
