# Multiplayer — Tactical Full-State Spine: ROADMAP + Inc2 Implementation Plan
...
> `docs\superpowers\specs\2026-06-20-multiplayer-tactical-full-state-replication-feasibility.md` (end-state)
> + `Multiplayer\docs\superpowers\specs\2026-06-19-multiplayer-tactical-generic-state-spine-design.md` (spine)
> + handoff `docs\superpowers\2026-06-20-multiplayer-consolidation-and-tactical-fullstate-handoff.md`.
> Roadmap tracker: `Multiplayer\docs\COOP-SYNC-ROADMAP.md`. MEMORY in force: `multiplayer-full-state-replication`,
> `dont-replace-working-architecture` (retire ONE surface per commit, revert-to-known-good on 3+ cascading
> fixes), `multiplayer-reconcile-existing-first`, `multiplayer-commit-workflow` (inner `main`, no push, no branches).
>
> Decompile proxy: `decompiled\AssemblyCSharp\Assembly-CSharp\src` (re-pin vs installed DLL before any patch).
> Tactical src: `Multiplayer\src\Sync\Tactical\*` + `Multiplayer\src\Harmony\Tactical\*`. Pure cores unit-tested
> (`Multiplayer.Tests`, linked per file); engine-glue is reflection-bound → NOT linked → in-game-gated.
> Build/test: `dotnet test Multiplayer\Multiplayer.Tests\Multiplayer.Tests.csproj`; mod build `Multiplayer.csproj`;
> deploy `Multiplayer\deploy.ps1` → `D:\Steam\...\Mods\Multiplayer` (junction covers `D:\PP-Instance2`).
...
## 2. Current state inventory (grounded — `Multiplayer\src\Sync\Tactical\TacticalSyncSurfaces.cs`)
...
ADDITIVE-first, retire only after in-game OK. Diagnostic logs `[Multiplayer][tac]`; strip before publish.
...
|---|---|---|
| `Multiplayer\src\Sync\Tactical\TacticalLiveCodec.cs` | **MODIFY** — encode/decode the `ActorFieldFacing 0x0010` field (FacingX/Y/Z) in ascending bit order between Pos(0x0008) and Health(0x0020); add `FacingX/Y/Z` + `HasFacing` to `ActorStateRecord`; update wire-format comment | YES (`:147`) — codec tests run |
| `Multiplayer\src\Sync\Tactical\TacticalActorStateDiff.cs` | **MODIFY** — add pure `FacingEpsilon` const + `FacingChanged(...)` decision (client skip + host signature) | YES (`:156`) — diff tests run |
| `Multiplayer\Multiplayer.Tests\TacticalActorStateCodecTests.cs` | **MODIFY** — add facing round-trip + truncation tests | (test file) |
| `Multiplayer\Multiplayer.Tests\TacticalActorStateDiffTests.cs` | **MODIFY** — add `FacingChanged` tests | (test file) |
| `Multiplayer\src\Sync\Tactical\TacticalActorStateSync.cs` | **MODIFY** — host: read facing (`Rot`→forward), add `facingSig`, set the bit + fill FacingX/Y/Z; client: apply facing after stats | NO (engine glue, in-game) |
| `Multiplayer\src\Sync\Tactical\TacticalMoveSync.cs` | **MODIFY** — add client `ApplyMirrorFacing` + `TrySetForward`/`GetForward` helpers | NO (engine glue, in-game) |
...
**1b. Run-fail:** `dotnet test Multiplayer\Multiplayer.Tests\Multiplayer.Tests.csproj` → these 4 fail to COMPILE
...
**1e. Commit** (commit-on-green workflow — write sentinel, `git -C Multiplayer add -A && git -C Multiplayer commit`,
delete sentinel, ONE Bash call):
`feat(multiplayer-tac): encode actor FACING field (0x0010) in tac.actorstate delta [Inc2]`
...
**2e. Commit:** `feat(multiplayer-tac): pure FacingChanged decision for actorstate delta [Inc2]`
...
**Commit:** `feat(multiplayer-tac): host ships actor FACING in tac.actorstate delta [Inc2]` (after `dotnet build
Multiplayer\Multiplayer.csproj` = 0 err/0 warn).
...
**Commit:** `feat(multiplayer-tac): client mirrors actor FACING from tac.actorstate delta [Inc2]` (after build
...
`multiplayer-second-instance-setup`. Watch `[Multiplayer][tac]` logs (`HOST broadcast tac.actorstate ... changedActors`,
...
If green → update `COOP-SYNC-ROADMAP.md` (Inc2 DONE) + memory `multiplayer-mod-status.md`, then proceed to Inc3.
