# Multiplayer Sync Canon Rollout — Implementation Plan
...
**Tech Stack:** C# .NET 4.7.2, Harmony/AccessTools, xUnit. Build `dotnet build Multiplayer.csproj -c Release`; tests `dotnet test Multiplayer.Tests`; deploy `pwsh -NoProfile -File .\deploy.ps1`.
...
- `docs/superpowers/specs/2026-06-27-multiplayer-sync-canon-design.md` (full design; invariants §4, status contract §5, bug→invariant map §6, migration stance §7).
...
The RCA found **no clean pure unit surface**: the helper is pure engine reflection (`TacticalActor`/`IdleAbility`/`TacticalPerception` are live game types, absent from the `Multiplayer.Tests` assembly which only holds engine-free logic such as `TacticalActorStateDiff`). Per the canon brief, **do NOT fabricate an xUnit test** here. Verification = clean Release build + deploy + an in-game acceptance gate (the soldier visibly crouches at cover on the client + a confirming log line). If a genuinely pure helper later falls out, add a real test then — not before.
...
Run: `dotnet build Multiplayer.csproj -c Release`
...
Expected (final line): `Deployed Multiplayer to D:\Steam\steamapps\common\Phoenix Point\Mods\Multiplayer` (the script rebuilds Release, copies `Multiplayer.dll` + `.pdb` + `meta.json`).
...
3. **Log confirm (client):** `%USERPROFILE%\AppData\LocalLow\Snapshot Games Inc\Phoenix Point\Player.log` contains `[Multiplayer][tac] CLIENT re-derived cover pose at (...)` for that move, and NO `TryRederiveCoverPose failed` / `not found` errors.
...
```bash
git -C E:/DEV/PhoenixPoint/Multiplayer add -A
git -C E:/DEV/PhoenixPoint/Multiplayer commit -m "fix(tac): re-derive client cover/stance pose at move completion (bug A)
...
- **Test:** `Multiplayer.Tests/TacticalActorStateDiffTests.cs` — add 3 facts for the pure magnitude mapping.
...
**Files:**
- Test: `Multiplayer.Tests/TacticalActorStateDiffTests.cs`
...
Append to `Multiplayer.Tests/TacticalActorStateDiffTests.cs`, immediately before the final closing `}` of the class:
...
Run: `dotnet test Multiplayer.Tests --filter "FullyQualifiedName~Magnitude"`
...
Run: `dotnet test Multiplayer.Tests`
Expected: `Passed!  - Failed: 0` (includes the 3 new `Magnitude_*` facts; no regressions).

- [ ] **Step 12: Deploy to the game Mods folder**

Run: `pwsh -NoProfile -File .\deploy.ps1`
Expected (final line): `Deployed Multiplayer to D:\Steam\steamapps\common\Phoenix Point\Mods\Multiplayer`.
...
4. **Log confirm (client `Player.log`):** `[Multiplayer][tac] status mirror magnitude applied: ... value=20` for the bleed; NO `BleedStatus`/`NullReferenceException` in the status-mirror path; NO `status mirror failed`.
...
```bash
git -C E:/DEV/PhoenixPoint/Multiplayer add -A
git -C E:/DEV/PhoenixPoint/Multiplayer commit -m "fix(tac): statuses ride the 0x8F spine only, with display magnitude (bug C)
...
git -C E:/DEV/PhoenixPoint/Multiplayer log --oneline -1
```
