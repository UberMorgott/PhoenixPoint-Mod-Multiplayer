...
- `tools/RailCheck/Program.cs` is shared and already dirty. For each slice, create a reviewed zero-context patch against `git show HEAD:tools/RailCheck/Program.cs` containing only that slice's exact `Add(laws, () => L<n>_...Run())` lines; apply it with `git apply --cached --check --unidiff-zero <patch>` then `git apply --cached --unidiff-zero <patch>`. Verify `git diff --cached -- tools/RailCheck/Program.cs` contains only those registrations. Delete the temporary patch before commit. This is deterministic and noninteractive.
- Every new file-backed law is registered once, increments `tools/law-count.txt`, and contains `POSITIVE CONTROL` or `premise-changed`. L371-L396 map one-to-one to DWI-01..26. A slice is GREEN only when its production symbols exist and its assigned law executes the claimed boundary.
...
|---|---:|---:|---|
| DWI-01 creation-set entitlement | L371 | 2 | serialized membership + `CreateOccurrence` |
| DWI-02 ACK is not lifecycle | L372 | 1 | codec + reducer ACK transition |
| DWI-03 display gate | L373 | 5 | `MayPresent` + native arrival gate |
| DWI-04 durable host order | L374 | 4 | native save blob + reconnect snapshot reconstruction |
| DWI-05 preempt/resume | L375 | 6 | scheduler + checkpointed carrier swap |
| DWI-06 exactly-once shared choice/effect | L376 | 8 | durable effect transaction/recovery |
| DWI-07 per-player locked-result retention | L377 | 8 | lifecycle + open repaint |
| DWI-08 canonical result/reward after load | L378 | 4 | saved ledger reconstruction with reordered definitions |
| DWI-09 local offer Cancel | L379 | 9 | native Cancel capture + reducer |
| DWI-10 atomic Start successor | L380 | 9 | successor transaction + carrier registry |
| DWI-11 collaborative edits/repaint | L381 | 10 | existing Equip/Personnel funnels + revision/repaint |
| DWI-12 first-valid launch/no quorum | L382 | 14 | recoverable launch transaction + native result boundary |
| DWI-13 source departure | L383 | 12 | source callback/revalidation + repaint/teardown |
| DWI-14 linked completion cleanup | L384 | 13 | `OnMissionRewardApplied` adapter + generation links |
| DWI-15 sole carrier survives failure | L385 | 9 | store fault between successor/effect and teardown |
| DWI-16 revisions/reconnect | L386 | 2 | reducer monotonicity/tombstone precedence |
| DWI-17 tactical checkpoint/restore | L387 | 15 | `LevelTeardown` + loaded ledger + started Geoscape |
| DWI-18 exhaustive taxonomy | L388 | 5 | router-derived family table |
| DWI-19 compaction proof | L389 | 3 | save/journal/cursor/replay references |
| DWI-20 all carrier classes | L390 | 7 | occurrence-indexed `RemoveAllCarriers` |
| DWI-21 Deferred Back | L391 | 11 | deployment Back capture + revision gate |
| DWI-22 durable membership/no ACK | L392 | 2 | enrollment/removal reducer |
| DWI-23 no late-join history | L393 | 2 | enrollment/create transaction order |
| DWI-24 same-epoch reconnect | L394 | 16 | paged snapshot reconciliation |
| DWI-25 enrollment/create serialization | L395 | 2 | one host transaction sequencer |
| DWI-26 exact priority registry | L396 | 5 | closed raiser token + patched modal comparison |

---

### Task 1: Immutable identity, codec, and minimal lifecycle reducer

**Files:** create `src/Rail/DurableInboxModel.cs`, `src/Rail/DurableInboxCodec.cs`, `src/Rail/DurableInboxReducer.cs`; create L372; modify `Program.cs`, `law-count.txt`.

- [ ] Add RED L372 that decodes a real `TransportAck` and executes `DurableInboxReducer.Apply`; it must prove queued/open/read/dismissed/choice states are byte-for-byte unchanged while an explicit lifecycle command changes only its named member/occurrence. Positive controls detect ACK-as-read and event-id-only identity.
...
**Files:** modify model/reducer; create L371, L386, L392-L395; modify Program/count.

- [ ] RED laws execute one sequencer for enrollment, creation, reconnect, and authoritative removal. L371 must include enrolled active, disconnected, loading, tactical, and non-Geoscape epochs; retransmit dedupes; a new trigger is distinct; later epochs receive none.
...
**Files:** create `src/Rail/DurableInboxStore.cs`; create L389; modify reducer, Program/count.

- [ ] RED L389 keeps a tombstone while any save snapshot, journal record, peer cursor, incomplete snapshot, or wire replay can name it; no TTL. Positive control proves a time-based deletion fails.
...
**Files:** create `src/Rail/DurableInboxSave.cs`; modify `DurableInboxStore.cs`; create/complete L374 and L378; modify Program/count.
...
**Files:** create `src/Rail/DurableWindowRegistry.cs`; modify `GeoWindowCoverage.cs`, `WindowOrder.cs`, `MissionSync.cs`; create L373/L388/L396; modify Program/count.
...
- [ ] Gate/remove `MissionArrivalNav.Step -> view.LaunchMission`; it enqueues/revalidates priority only. L373 structurally walks the real call path and fails if arrival can invoke `LaunchMission` without `MayPresent`.
...
**Files:** create `src/Rail/DurableInboxEngine.cs`; modify `WindowOrder.cs`, `WindowQueueSync.cs`, `EventPopup.cs`, `GeoModalMirror.cs`; create L375; modify Program/count.

- [ ] RED L375 opens ordinary content with checkpoint, injects registered priority, and requires durable `Suspended(PriorityPreemption)` before replacement. Callback counts stay zero; same occurrence/result phase/selection/read checkpoint resumes; invalidated suspended content does not.
...
**Files:** modify `DurableInboxEngine.cs`, `EventPopup.cs`, `GeoModalMirror.cs`, `CutsceneMirror.cs`, `MissionOutcomeMirror.cs`, `WindowQueueSync.cs`, `DeploymentWindow.cs`; create L390; modify Program/count.

- [ ] RED L390 registers native current, native pending, mod queued/suspended/deferred, wire/replay, and tactical-held carriers for one occurrence. Deleting current only is the positive-control failure.
...
**Files:** modify reducer/store/engine, `EventSync.cs`, `EventPopup.cs`, `UiEventMap.cs`; create L376/L377; strengthen L378; modify Program/count.
...
**Files:** modify reducer/store/engine, `MissionSync.cs`, `WindowQueueSync.cs`; create L379/L380/L385; modify Program/count.
...
**Files:** modify `EquipSync.cs`, `PersonnelSync.cs`, reducer/engine, `UiEventMap.cs`, `OpenUiRepaint.cs`, `DeploymentWindow.cs`; create L381; modify Program/count.
...
**Files:** modify reducer/engine, `DeploymentWindow.cs`, `WindowOrder.cs`; create L391; modify Program/count.
...
**Files:** modify reducer/engine, `MissionSync.cs`, `DeploymentWindow.cs`; create L383; modify Program/count.
...
**Files:** modify reducer/engine, `MissionOutcomeMirror.cs`, `MissionSync.cs`; create L384; modify Program/count.
...
- [ ] L384 structurally proves the patch reaches the reducer and behaviorally rejects cleanup by mission ID alone. A completion outcome remains deliverable.
...
**Files:** modify reducer/store/engine, `MissionSync.cs`, `DeploymentWindow.cs`; create L382; strengthen L355; modify Program/count.
...
- [ ] Remove countdown/veto/readiness/cancel-launch authority. L382 injects native failure after `Launching` persistence but before boundary announcement and proves `Preparing` remains retryable; AFK peers never block.
...
**Files:** modify `LevelTeardown.cs`, `SaveTransferCoordinator.cs`, store/engine, `ReplenishSync.cs`; create L387; modify Program/count.
...
**Files:** modify codec/engine, `WindowQueueSync.cs`, `SyncEngineStub.cs`, `SessionManager.cs`; strengthen L372/L386/L394; no `SurfaceIds` or `SyncProtocol` behavior change.
...
**Files:** modify `EventPopup.cs`, `EventSync.cs`, `GeoModalMirror.cs`, `CutsceneMirror.cs`, `MissionOutcomeMirror.cs`, `MarketplaceSync.cs`, `WindowOrder.cs`, `ReplenishSync.cs`, store/engine; strengthen L371/L374/L378/L385/L388.

- [ ] Convert every Task 5 durable family capture to `OccurrenceDraft` with persisted trigger; explicitly leave classified local-only families outside DWI. New router families must break L388 until classified.
...
**Files:** all L371-L396 and relevant existing laws; then `docs/laws.md`, `README.md`, design status line only.
...
- [ ] Run exact integrity/build/RailCheck chain. Expected final count is the then-current baseline plus exactly 26 file-backed laws; derive it from `tools/law-count.txt` at Task 1 rather than trusting stale `files=192/inline=60` literals. Every L371-L396 is registered exactly once with a positive control.
...
- [ ] Matrix maps DWI-01..26 exactly once to L371-L396 and a final GREEN task; codec-only tests do not claim save/load or entitlement behavior.
- [ ] No placeholders remain: scan for `TODO|TBD|FIXME|<EXPLICIT_PATHS>|<PROGRAM_PATCH>|<COMMIT_MESSAGE>|X:\\path|files=192|EncodeEnvelope\(byte, byte, uint` and allow only the three documented commit-template tokens inside the generic template, never inside a task command.
- [ ] Type/signature scan confirms `CanonicalResultId`, immutable copied subject equality, `DurableInboxReducer` in Task 1 for L372, and only `EncodeEnvelope(byte, SyncKind, byte[])`.
