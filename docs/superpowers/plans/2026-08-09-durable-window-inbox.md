...
- [ ] Model `Unanswered -> EffectPending(effectToken, canonical choice/result/reward inputs, serialized campaign RNG state, winner) -> EffectApplied -> ChoiceLocked` through the spec's two-checkpoint rollback transaction. Generated identities become concrete in POST rather than being falsely claimed before generation. Persist and verify a unique PRE campaign checkpoint before synchronous native effect execution; freeze simulation/input/intents/outbound deltas and defer every non-owner save for the whole iterator lifetime; persist and verify a unique POST checkpoint before response/unfreeze. Recovery selects POST or authoritatively reloads PRE, never continues a partial in-memory world. Gameplay-visible external non-save effects fail closed; non-authoritative telemetry is outside occurrence idempotence.
