# Multiplayer — Host→Client Report-Window Mirror (implementation plan)
...
Spec: `Multiplayer\docs\superpowers\specs\2026-06-26-multiplayer-unified-sync-backbone-design.md`.
...
- Host Postfix broadcast + client Prefix suppress: `EventRaisedDisplayPatch`
  (`Multiplayer\src\Harmony\Sync\EventDisplayPatches.cs:30-118`). Prefix returns `true` when
...
- Broadcast: `SyncEngine.BroadcastEventRaised` (`Multiplayer\src\Network\Sync\SyncEngine.cs:488`,
...
- Client apply: `SyncEngine.OnEventRaised` (:336) / `OnEventDismiss` (:419) → reconstruct refs by
  id → `EventDisplay.Show/ShowResult/Dismiss` (`Multiplayer\src\Network\Sync\State\EventDisplay.cs`)
...
- Hard client suppression of LOCAL raises: `ClientEventRaiseChokepointPatch`
  (`Multiplayer\src\Harmony\Sync\EventRaiseChokepointPatch.cs`) + `SuppressEvents` flag.
- Reusable reward-line display snapshot (text-only, no re-apply): `RewardDisplaySnapshot` /
  `RewardDisplayReflection` (`Multiplayer\src\Network\Sync\State\`), already used for event result cards.
...
- Gate precedent: `GeoRailGate.Enabled` (`Multiplayer\src\Network\Sync\GeoRailGate.cs`) + default
  test `Multiplayer.Tests\GeoRailGateTests.cs` (xUnit).
- Free `PacketType` byte: 0x67 `SyncEnvelope`, **0x68 retired (do-NOT-reuse)** → next free **0x69**
  (`Multiplayer\src\Network\MessageLayer\PacketType.cs:58-68`).
...
New file `Multiplayer\src\Harmony\Sync\ReportModalMirrorPatches.cs`, two `[HarmonyPatch]` on the SAME
...
### 4.6 Gate
`Multiplayer\src\Network\Sync\ReportMirrorGate.cs`:
```csharp
...
10. Strip/guard `[Multiplayer]` DIAG logs to one concise line per broadcast/apply (spec §9).
