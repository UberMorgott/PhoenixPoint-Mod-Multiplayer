# Multiplayer Tactical Full-Mirror Arc — Session Handoff 2026-06-20 NIGHT
...
2. **Read the overwatch delta** in client log: `[Multiplayer][tac] overwatch arm: ... delta=` — large value = re-anchor fix `809fb09` confirmed working; ~0 = dig LOS/weapon gate `TacticalLevelController.cs:1394`.
...
- **A:** client enemy turn -> bar hides, banner, camera follows mobs; handoff back -> HUD returns; fog ok; log `[Multiplayer][tac] CLIENT entered ENEMY-TURN presentation`.
...
- **(overwatch)** client log `[Multiplayer][tac] overwatch arm: ... delta=` — big = re-anchor fix `809fb09` worked, ~0 = dig LOS-gate `TacticalLevelController.cs:1394`.
...
- deploy.ps1 -> `D:\Steam\steamapps\common\Phoenix Point\Mods\Multiplayer`.

---

## Key files

All under `Multiplayer\src\Sync\Tactical\` unless noted:
