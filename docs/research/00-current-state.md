...
### Co-op loading screen — two-phase barrier + synchronized reveal (latest, deployed; ITEM 3 + visuals in-game-pending) — co-op load CONFIRMED WORKING over DirectIP

**CO-OP LOAD CONFIRMED WORKING over DirectIP (in-game):** phase-1 save transfer + LOADED/BEGIN barrier + phase-2 native-bar overlay all function on the reliable DirectIP transport. Native progress bars + self-hidden + named rows verified.

**KNOWN LIMITATION (deferred): STUN/UDP save transfer is UNRELIABLE.** Same transfer over the STUN/WAN UDP transport is best-effort — 32KB chunks fragment over UDP with NO ACK/retransmit; one lost fragment fails the transfer → client never completes the save → never sends `LOADED` → phase-1 barrier sticks at `loadedPeers=1/expected=2`, load never starts. Diagnosed via phase-1 instrumentation (commit `61582b4`). FIX (deferred): add an ACK/retransmit layer on `StunTransport`, OR use DirectIP.

Feature commit (latest on `main`, deployed + hash-verified, NOT pushed): overlay polish (self-hidden + nickname rows) + simultaneous reveal IMPLEMENTED — see "ITEM 3" below.
...
- **B+C — phase-2 progress was gated on overlay visibility (host aggregate stalled).**
  ...
  - Fix: Unity-free `RosterProgressTracker.InPhase2(bool begun,bool loadCompleteSent) => begun && !loadCompleteSent` (`RosterProgressTracker.cs:64`) + coordinator prop `SaveTransferCoordinator.InPhase2` (`:135`). The progress pump moved to the TOP of `SaveTransferCoordinator.Update()`, ABOVE the host-only early-return (`if (!_engine.IsHost || (!_barrierOpen && !_loadPhaseActive)) return;`). Runs on every peer each frame (NetworkEngine.Update → SaveTransfer.Update, unconditional). `_lastReportedLoadPct` field moved controller → coordinator. `LoadOverlayController.Update()` is now UI-refresh-only.
- **Latest co-op-load fix (on `main`, deployed + hash-verified, NOT pushed, in-game pending) — STATIC phase-2 bar root cause + native-bar reuse + client-show.**
  - **ROOT CAUSE (corrects the prior claim):** the phase-2 bar was STATIC because `GameUtl.CurrentLevel()` returns NULL during a geoscape load — the old level is cleared at `Game.cs:191`, the new one assigned only at `Game.cs:211` AFTER `LoadCrt` finishes. The pump read null every frame, never reported real phase-1 progress (bar held the phase-0 download=100%). FIX: pump now reads the actual LOADING `Level` instance, captured from the curtain hook `CurtainShowPatch.Postfix(object level, object prevState, object newState)` → `SaveTransferCoordinator.SetLoadingLevel(level)` on "Loading" (cleared on "Playing"/"Loaded"), then reads `_loadingLevel.LoadingProgress.Progress`. The native loading bar binds the same way (`SetLoadingLevel(level)` → `level.LoadingProgress`), which is why IT shows real % while `CurrentLevel()` is null.
  - **BUG B (client never showed overlay):** `TransferActive` is false by the time the client curtain hits "Loading" (`_rxTotalBytes`=0 after ResetRx; `IsBarrierPending`=false because `_begun` set in EnterLevel before FinishLevel). The `CurtainShowPatch` "Loading" gate is now `!(coord.TransferActive || coord.InPhase2)` so the client shows during phase-2.
  - **Native-bar reuse (visual change):** overlay rows are NO LONGER custom from-code Images — they are CLONES of the native `Base.Utils.ProgressBarController` (vanilla loading bar; `Image ProgressFill` fillAmount + `Text ProgressText`), captured via `NativeWidgetFactory.CaptureLoadingBarTemplate()` (`FindObjectOfType` SceneFadeController → `ProgressBar` field → gameObject). One small bar per player TOP-RIGHT, name `Text` label. Cloned `ProgressBarController` component DISABLED (its own Update ramp doesn't fight us); driven manually `ProgressFill.fillAmount = Tracker.Get(slot).percent/100`. BOTH local + remote rows driven by tracker; ALL players shown INCLUDING self (self-row-skip removed). NULL-template fallback to old from-code Image row preserved.
  - **Cover REMOVED (regression):** the prior fullscreen black "Cover" Image blacked out the host's loading screen — removed.
  - **`RosterProgress 0x1D`** = host-aggregated per-slot snapshot (unreliable); `LoadProgress 0x1A`; `RevealAll 0x1F` (reliable second-barrier reveal). slotIndex-keyed tracker (NEVER LocalSteamId — 2-instance same-machine rig).
  - **ITEM 3 — overlay polish + simultaneous reveal NOW IMPLEMENTED (no longer designed/HELD).**
    - **Self-row hidden again:** `LoadOverlayController.Refresh` skips `SlotIndex == Session.LocalSlotIndex` → overlay shows ONLY OTHER players.
    - **Named bars:** each bar shows player NICKNAME from `PeerListEntry.Nickname` via `GetLobbyRoster()` (populated for remotes by host's authoritative `BroadcastPeerList`); fallback `"Player {slot}"`.
    - **Simultaneous geoscape reveal via native-curtain-hold (held design now done):** Harmony PREFIX on `LevelSwitchCurtainController.OnLevelStateChanged` returns `false` ONLY on `Loaded→Playing` while `engine.IsActive && SaveTransfer.SessionStarted && !Revealed` — suppresses the native auto-`LiftCurtain` so every peer HOLDS at the still-visible native loading screen (NOT a black cover). Postfix still fires `OnReachedPlaying` (hold + idempotent `SendLoadComplete`). Host collects every `LoadComplete 0x1E`; on `_tracker.AllDone(GetRosterSlots())` broadcasts `RevealAll 0x1F`; every peer's `OnRevealAll → PerformDeferredLift` lifts together. `PerformDeferredLift` now reflection-invokes native `LiftCurtain()` (animated alpha→0, unpauses rendering, fires `OnCurtainLifted → GeoscapeView` restores input+sound) then `HideLoadOverlay`, behind the `_revealed` once-guard. New `SaveTransferCoordinator.Revealed` getter.
    - **DEADLOCK-SAFE:** suppression gated on `!Revealed`; every `_revealed` path lifts the native curtain. Held client with dead host self-reveals after `BarrierTimeoutMs` (per-peer fallback, runs on clients above the host-only return); a peer erroring before Playing is covered by host forced-reveal at `_phase2DeadlineMs` (Begin+60s). No reachable stuck-suppressed-curtain state. REVIEWER ship-cleared.
  - **MultipleerLog** now falls back to `multipleer-2.log`..`-5.log` on IOException so the 2nd same-machine instance (client) gets its own dedicated log instead of dumping into Player.log.
  - Instrumentation: `[Multipleer]` log lines at the show predicate, phase-2 pump tick (with pct), RosterProgress SEND/RECV, OnReachedPlaying/PerformDeferredLift/AllDone→RevealAll.
  - **Visual smoothing — bar ease (on `main`, deployed; in-game-pending). Tests 90/90.** SYMPTOM: native co-op bar jumped to ~70%, sat ~4.5s, then to done — not smooth. ROOT CAUSE (log-confirmed): source `_loadingLevel.LoadingProgress.Progress` is coarse/step-quantized — phase-2 pump reports only `pct = 0→40→69→80`→done (geoscape build = a few big steps); bar rendered them honestly. Clone inertness, RosterProgress ~5Hz cadence, phase→fill mapping all CORRECT — not the cause. FIX (client-side, UI-only): new pure Unity-free `FillEase.EaseFill(current,target,maxStep)` (monotonic-up, clamp 0..1, no overshoot) — `src/UI/FillEase.cs`, 5 unit tests `Multipleer.Tests/FillEaseTests.cs`. `LoadOverlayController.Refresh` now eases each row's `DisplayFill` toward `percent/100` at `FillEaseRate=0.6`/s (`MoveTowards`-style step `FillEaseRate*Time.deltaTime`), drives `fillAmount=DisplayFill` + `%=RoundToInt(DisplayFill*100)` on BOTH native + fallback rows; cloned `ProgressBarController` stays disabled (we drive the eased fill). `DisplayFill` reset to 0 in `Show()` so a 2nd co-op load animates from 0. KNOWN: source stays coarse (engine exposes no fine 0..1 for geoscape build) — ease makes it LOOK smooth; data granularity itself is an engine limitation (not fixable without a finer native progress hook).
...
- **Pending / to re-test** (per `multipleer-second-instance-setup`):
  ...
  - Co-op load over **DirectIP — CONFIRMED WORKING in-game ✓**: phase-1 transfer + LOADED/BEGIN barrier + phase-2 native-bar overlay (B/C + native progress bars, self-hidden, named). STUN/UDP transfer **UNRELIABLE — known limitation (deferred)**, needs ACK/retransmit on `StunTransport` (diagnosed via `61582b4`).
  - ITEM 3 (latest feature commit, on `main`, not pushed) — overlay self-hidden + nickname rows + **simultaneous-reveal via native-curtain-hold** (prefix-suppress `LevelSwitchCurtainController.OnLevelStateChanged` + deferred `LiftCurtain()` on `RevealAll 0x1F`, deadlock-safe) — **in-game pending**.
