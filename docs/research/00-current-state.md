...
- **Latest co-op-load fix (on `main`, deployed + hash-verified, NOT pushed, in-game pending) — STATIC phase-2 bar root cause + native-bar reuse + client-show.**
  ...
  - **D simultaneous-reveal via native-curtain-hold = DESIGNED but HELD** (not yet implemented) pending in-game B/C verification.
...
- **Pending / to re-test** (per `multipleer-second-instance-setup`):
  ...
  - Latest co-op-load fix (on `main`, not pushed) — STATIC phase-2 bar root cause (`CurrentLevel()`-null → captured loading `Level` via `SetLoadingLevel`), client-show predicate `!(TransferActive||InPhase2)`, native `ProgressBarController` clone rows (all players incl. self), Cover removed, MultipleerLog 2nd-instance fallback — **in-game pending**.
