...
namespace Multipleer.Network.CommandSync
{
    ...
    // re-emit. DIAG3: one-shot-per-identity post-apply nav log (resolved-or-not + Surface.position /
    // Travelling / CurrentSite) to confirm the mirror actually wrote vs NavigateRoutine fighting it.
    ...
    public static class ClientGeoStateApplier
    {
        ...
        // One-shot DIAG3 nav-log gate per identity (Phase-B revert removes the logging; this stays harmless).
        private static readonly HashSet<(string, int)> _diag3Logged = new HashSet<(string, int)>();
        ...
        private static void ApplyVehicleRecord(object geoLevel, GeoVehicleStateRecord record)
        {
            ...
            // DIAG3: one-shot-per-identity post-apply nav log — confirms the mirror WROTE the host's values
            // (vs a client NavigateRoutine fighting it). Reads back the same accessors GeoBridge writes.
            if (_diag3Logged.Add(identity))
            {
