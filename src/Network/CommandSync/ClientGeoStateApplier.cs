...
namespace Multipleer.Network.CommandSync
{
    ...
    public static class ClientGeoStateApplier
    {
        ...
        private static void ApplyVehicleRecord(object geoLevel, GeoVehicleStateRecord record)
        {
            ...
            //
            // INC-A (single-writer mirror): the client runs NO native vehicle travel routine. The on-globe icon
            // is placed RAW at the latest applied transform (via ClientVehicleInterpolator) for EVERY vehicle,
            // regardless of who initiated travel — host stays sole authority and pushes 0x35 state only.
