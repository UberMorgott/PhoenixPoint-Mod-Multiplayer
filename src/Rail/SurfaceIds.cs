namespace Multiplayer.Network.Sync
{
    ...
    public static class SurfaceIds
    {
        ...
        public const byte GeoItemClaim = 0xAF;  // EPHEMERAL drag-claim advisory (ClaimSync): [op:u8 claim/release/releaseAll][token:u32][itemDefGuid]; client→host (ActionRequest) relayed host→all (StateDelta); in-memory only, TTL backstop, NEVER on the persistent 0xAC value rail
    }
}
