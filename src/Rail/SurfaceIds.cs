namespace Multiplayer.Network.Sync
{
    ...
    public static class SurfaceIds
    {
        ...
        public const byte GeoScrapCart = 0xB2;  // Shared scrap-cart ECHO host→all ([seq:u32][nItems:u16][(defGuid,count:u16)×N][nVeh:u16][(defGuid,count:u16)×M]) — the host-authoritative pending-scrap store (ManufactureSync). Gestures ride 0xAE as OpCartAdd/OpCartRemove/OpCartScrap; the native cart is per-screen UI state the game clears on reopen (UIModuleManufacturing.Init:363-371), so the shared truth lives in the mod store and is spliced into open screens. seq rides SurfaceSeq; no client→host traffic on this surface.
    }
}
