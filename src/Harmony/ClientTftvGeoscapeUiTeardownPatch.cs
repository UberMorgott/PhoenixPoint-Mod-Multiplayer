...
{
    // Save-load robustness (TFTV compat). On a co-op CLIENT, applying a host save-transfer forces a full
    // geoscape teardown+rebuild (SaveTransferCoordinator -> EnterLevel -> FinishLevel). During that window
    // two TFTV geoscape-UI methods run on null backing data and throw a NullReferenceException, surfacing
    // TFTV's own error dialog. Multipleer is only the TRIGGER (the exception is 100% inside TFTV code).
    //
    // These two PREFIX patches skip the TFTV body during that one teardown window ONLY -- a co-op client
    // whose GameUtl.CurrentLevel() is null. Outside that window (single-player, host, or an active client in
    // normal play with a live level) the prefix returns true and TFTV's food/mutagen + ODI UI run unchanged.
    //
    ...
    internal static class ClientTftvGeoscapeUiTeardownDecision
    {
        ...
        // Best-effort + fail-OPEN: any failure reading state -> let TFTV run (its own try/catch handles it),
        // so this guard can never throw into the game.
