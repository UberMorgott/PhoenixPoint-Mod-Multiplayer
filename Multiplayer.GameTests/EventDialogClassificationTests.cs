...
public class EventDialogClassificationTests
{
    ...
    // ─── mission-deploy exclusion (SOURCE-side: host never broadcasts a PURE arrival deploy/land prompt) ──
    // The classification moved to the pure Multiplayer.Core MissionDeployClassifier.IsPureDeployPrompt
    // (2026-07-13 narrowing of the 9e80b24 ANY-classifier: only mission-choice + bare-decline windows are
    // excluded; story events mixing a mission choice with rewarded alternatives mirror again). Its truth
    // table is pinned in Multiplayer.Tests\MissionDeployClassifierTests.cs (the always-run pure gate). The
    // reflection readers (IsMissionDeployEvent / ChoiceHasOutcomePayload) bind live game types and cannot
    // JIT in-process — reasoned-correct from the decompile (GeoEventChoiceOutcome.cs:25-105/315).
}
